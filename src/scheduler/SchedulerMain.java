...
public class SchedulerMain{
	...
	public void createNewGenerations(){
		...
		while(nogenerations<maxgenerations){	
			...
			//first 1/10 chromosomes added as it is- Elitism 
			for(i=0;i<populationsize/10;i++){	// Elitism means that the most fit handful of individuals are guaranteed a place in the next generation, generally without undergoing mutation. It suggests duplicating the most fit individual-that individual gets two reserved slots in the next generation. One of these slots is mutated, the other is not.
				newlist.add(firstlist.get(i).deepClone());	// selecting first 1/10th of chromosomes in a population for elitism		
				newlistfitness+=firstlist.get(i).getFitness(); // fitness of all first 1/10th selected chromosomes is added
			}
			
			//adding other members after performing crossover and mutation
			while(i<populationsize){  // performing crossover and mutation of all chromosomes in a population leaving the first 1/10th.
				...
				newlist.add(son);  // adding the rest chromosomes in the population to the list.
				newlistfitness+=son.getFitness();  // sum of fitnesses of 1/10th selected chromosomes for 
				i++;                              // elitism + fitness of all rest chromosomes(son after crossover and mutation)
				
			}
			// if required fitness found print the timetable or else print the generation.
			//if chromosome with fitness 1 found
			if(i<populationsize){
				
				System.out.println("****************************************************************************************");
				System.out.println("\n\nSuitable Timetable has been generated in the "+i+"th Chromosome of "+(nogenerations+2)+" generation with fitness 1.");
				System.out.println("\nGenerated Timetable is:");   // nogenerations+2 = nogenerations+1 (total generations completed) + 1 (current generation counted at line no 110)
				son.printTimeTableSchedule();
				finalson=son;
				break;
			...
			//if chromosome with required fitness not found in this generation
			firstlist=newlist;
			Collections.sort(newlist);Collections.sort(firstlist);   // sort by fitness
			System.out.println("**************************     Generation"+(nogenerations+2)+"     ********************************************\n");
			printGeneration(newlist);
			nogenerations++;
	...
	public void customMutation(Chromosome c){
		...
		double newfitness=0,oldfitness=c.getFitness();
		int geneno=new Random().nextInt(inputdata.notimetable);
	...
	public Chromosome crossover(Chromosome father,Chromosome mother){
		...
		int randomint=new Random().nextInt(inputdata.notimetable);
		Gene temp=father.gene[randomint].deepClone();
		father.gene[randomint]=mother.gene[randomint].deepClone();
		mother.gene[randomint]=temp;
		if(father.getFitness()>mother.getFitness())return father;
	...
	public void initialisePopulation(){
		...
		for(int i=0;i<populationsize;i++){
			...
			Chromosome c;
			firstlist.add(c=new Chromosome());
			firstlistfitness+=c.fitness;    // total fitness of all chromosomes in first generation
			
		}
		Collections.sort(firstlist);    // sorting initial population by fitness and printing
		System.out.println("----------Initial Generation-----------\n");
		printGeneration(firstlist);
